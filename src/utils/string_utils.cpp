...
size_t write_to_string(const char *ptr, size_t size, size_t nmemb,
                       void *userdata) {
...
std::string trim(std::string s) {
  ...
  s.erase(std::find_if(s.rbegin(), s.rend(),
                       [&](unsigned char c) { return !isspace2(c); })
              .base(),
          s.end());
  return s;
}

std::filesystem::path append_to_path(const std::filesystem::path &path,
                                     std::string str) {
  return std::filesystem::path(path.string() + str);
}

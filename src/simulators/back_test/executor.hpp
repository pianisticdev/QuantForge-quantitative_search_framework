...
namespace simulators {
    ...
    class Executor {
       public:
        ...
        [[nodiscard]] static Money calculate_margin_required(const plugins::manifest::HostParams& host_params, Money fill_price,
                                                             double position_opening_quantity, double leverage);

        [[nodiscard]] static ClosingMarginInfo calculate_closing_margin_info(const models::Order& order, Money fill_price, double position_closing_quantity,
                                                                             const simulators::State& state);
    };
